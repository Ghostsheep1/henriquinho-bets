...
- `GET /api/cron/odds-snapshot`: protected scheduler route. It makes the single paid request to The Odds API and requires `CRON_SECRET`. Vercel Hobby only allows one daily cron, so `.github/workflows/odds-snapshot.yml` invokes this endpoint every two hours instead.
...
- The provider's free plan currently has 500 credits per month. The odds endpoint is charged by `regions x unique markets`; `upcoming + us + h2h` costs **1 credit per scheduled refresh**. Twelve two-hour scheduled refreshes per day use about **360 credits in 30 days** and **372 credits in 31 days**, leaving at least the configured 100-credit reserve. Verify terms at [The Odds API pricing](https://the-odds-api.com/) and [v4 guide](https://the-odds-api.com/liveapi/guides/v4/).
- The scheduler route stops when the daily limit reaches 12, quota is exhausted, another job is running, or the provider reports 100 credits or fewer. It does not retry quota errors. Players consume the existing snapshot only. GitHub Actions authenticates with its signed OpenID Connect token, bound to this repository, the `main` branch, and this workflow—no copied GitHub secret is required. `CRON_SECRET` remains available only for manual/internal calls.
