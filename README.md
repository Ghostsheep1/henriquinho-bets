...
- The scheduler route stops when the daily limit reaches 12, quota is exhausted, another job is running, or the provider reports 100 credits or fewer. It does not retry quota errors. Players consume the existing snapshot only. Configure GitHub repository secrets `HENRIQUINHO_SNAPSHOT_CRON_URL=https://henriquinhobets.vercel.app/api/cron/odds-snapshot` and `HENRIQUINHO_CRON_SECRET` with the same value as Vercel's `CRON_SECRET`.
